     ...
     char *str1, *str2;
{
  ...
  char *s1,*s2;
  if (cmp_debug_flag)
    printf("featcmp str1 = %s str2 = %s\n",str1,str2);
  if(str1==str2)
    {
      if (cmp_debug_flag)
	printf("featcmp return A 0\n");
      return 0;
    }
  ...
  if(*(str1+1)==0 && *(str2+1)==0)
    {
      if (cmp_debug_flag)
	printf("featcmp return B %ld\n",(long)( *str1 - *str2));
      return (long)(*str1 - *str2);
    }
  ...
  if (is_int(&s1,&len1,&sgn1)) {
	...
	{
	  if (cmp_debug_flag)
	    printf("featcmp return C %ld\n",sgn2-sgn1);
	  return (sgn2-sgn1); /* Check signs first */
	}
      if (len1!=len2) {
	if (cmp_debug_flag)
	  printf("featcmp return D %ld\n",len1-len2);
	return (len1-len2); /* Then check lengths */
      }
      {
	if (cmp_debug_flag)
	  printf("featcmp return E %ld\n",lf_strcmp(s1,s2));
	
	return lf_strcmp(s1,s2); /* Use strcmp only if same sign and length */
      }
    }
    else {
      if (cmp_debug_flag)
	printf("featcmp return F -1\n");
      return -1;
    }
  }
  else {
    if (is_int(&s2,&len2,&sgn2))
      {
	if (cmp_debug_flag)
	  printf("featcmp return G -=1\n");
	return 1;
      }
    else {
      if (cmp_debug_flag)
	printf("featcmp return H %ld\n",lf_strcmp(s1,s2));
      return lf_strcmp(s1,s2);
    }
  }
}
...
/******** GENERAL_INSERT(comp,keystr,tree,info,heapflag,copystr,bkflag)
  ...
  possible values: COMP==lf_strcmp(), COMP==intcmp(), or COMP==featcmp().
...
long heapflag, copystr, bkflag;
{
  ...
  if (comp == INTCMP) {
    cmp=intcmp(keystr,(*tree)->key);
  }  
  else if (comp ==FEATCMP) {
    cmp=featcmp(keystr,(*tree)->key);
  } 
  else if (comp ==STRCMP) {
    cmp=lf_strcmp(keystr,(*tree)->key);
  } 
  else
...
GENERIC info;
{
  general_insert(FEATCMP,keystr,tree,info,HEAP,TRUE,0L);
}
...
GENERIC info;
{
  ...
  general_insert(FEATCMP,keystr,tree,info,STACK,TRUE,0L);
}
