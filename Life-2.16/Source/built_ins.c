     ...
     ptr_psi_term tail;
{
  ...
  cons->type=alist;
  if(head)
    stack_insert(FEATCMP,(GENERIC)one,&(cons->attr_list),(GENERIC)head);
  if(tail)
    stack_insert(FEATCMP,(GENERIC)two,&(cons->attr_list),(GENERIC)tail);
     ...
     ptr_psi_term right;
{
  ...
  pair->type=and;
  if(left)
    stack_insert(FEATCMP,(GENERIC)one,&(pair->attr_list),(GENERIC)left);
  if(right)
    stack_insert(FEATCMP,(GENERIC)two,&(pair->attr_list),(GENERIC)right);
...

{
  ...
  if (arg2 && arg1) {
	...
	if((GENERIC)arg1>=heap_pointer) { /*  RM: Feb  9 1993  */
	  ...
	  heap_insert(FEATCMP,(GENERIC)label,&(arg1->attr_list),(GENERIC)result->coref);
	}
	else {
	  ...
	  if(arg1->type->always_check || arg1->attr_list)
	    bk_stack_insert(FEATCMP,(GENERIC)label,&(arg1->attr_list),(GENERIC)result);
	  else {
	    bk_stack_insert(FEATCMP,(GENERIC)label,&(arg1->attr_list),(GENERIC)result);
