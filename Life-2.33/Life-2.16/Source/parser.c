...
void feature_insert(keystr,tree,psi)
char *keystr;
ptr_node *tree;
ptr_psi_term psi;
{
...

{
  ...
  if (parse_ok) {
	...
	if (stringparse) parse_ok=FALSE;
      ...
      result.type=typ;
      if(car)
	stack_insert(FEATCMP,(GENERIC)one,&(result.attr_list),(GENERIC)car);
      if(cdr)
	stack_insert(FEATCMP,(GENERIC)two,&(result.attr_list),(GENERIC)cdr);
    }
  }
...
psi_term read_psi_term()
{
  ...
  if(t.type==variable && t.attr_list) {
    ...
    t.coref=NULL;
    t.resid=NULL;
    stack_insert(FEATCMP,(GENERIC)functor->keyword->symbol,
		 &(t.attr_list),
  ...
  if((t.type==add_module1 || t.type==add_module2 || t.type==add_module3) &&
    ...
    stack_insert(FEATCMP,(GENERIC)two,&(t.attr_list),(GENERIC)module);
  }
...
{  
  ...
  if (/* UNI FALSE */ equ_tokch((*tok),':') && arg1 && arg2) {
    ...
    stack_insert(FEATCMP,(GENERIC)one,&(tok->attr_list),(GENERIC)stack_copy_psi_term(*arg1));
    if (arg2)
      stack_insert(FEATCMP,(GENERIC)two,&(tok->attr_list),(GENERIC)stack_copy_psi_term(*arg2));
  }
