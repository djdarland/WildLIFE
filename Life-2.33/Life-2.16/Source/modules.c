...
extern char legal_in_name(char);
...
long lf_strcmp(char * s1,char *s2)
{
  long c;
  c = (long)strcmp(s1,s2);
  if (cmp_debug_flag)
    printf("lf_strcmp s1 = %s s2 = %s ret = %ld\n",s1,s2,c);
  return(c);
}
     ...
     char *module;
{
  ...
  if(!new) {
    ...
    heap_insert(STRCMP,(GENERIC)new->module_name,&module_table,(GENERIC)new);
...
void open_module_tree(n, onefailed)
ptr_node n;
int *onefailed;
{
  if (n) {
...
void open_module_one(t, onefailed)
ptr_psi_term t;
int *onefailed;
{
     ...
     
{
  ...
  if(!lf_strcmp(old_attr->key,oldlabel))
    stack_insert(FEATCMP,(GENERIC)newlabel,&(term->attr_list),(GENERIC)value);
  else
    stack_insert(FEATCMP,(GENERIC)old_attr->key,&(term->attr_list),(GENERIC)value);
