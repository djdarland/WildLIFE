      ...
      ptr_psi_term *t;
 {
   ...
   while (unchecked(t,sizeof(psi_term),29)) {
	 ...
	 if (sub_type((*t)->type,real))
       ...
       // I restored change I made ealier(2.06) --- 2.16 - It fixes GC error DJD
	else if (sub_type((*t)->type,sys_bytedata)) {
	  if (((unsigned long) * ((*t)->value)) > 100000) { // Practical test DJD
	    //	  fprintf(stderr,"pre 2 check_bytedata DJD %lx val %lx %lx %ld\n",(unsigned long)&((*t)->value),(unsigned long)((*t)->value),(unsigned long)* ((*t)->value),  (unsigned long) * (GENERIC *) * ((*t)->value)) ;
	  check_bytedata((unsigned long) * ((*t)->value),2);
	  }
	  else {
	    //	  fprintf(stderr,"pre check_bytedata DJD %lx val %lx %lx %lx\n",(unsigned long)&((*t)->value),(unsigned long)((*t)->value),(unsigned long)* ((*t)->value),  (unsigned long) * ((*t)->value)) ;
	  check_bytedata(((*t)->value),2);
	  }
	}
 #ifdef CLIFE
