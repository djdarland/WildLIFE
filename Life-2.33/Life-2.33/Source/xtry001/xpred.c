/*! \file xpred.c
  \brief x windows predicates

  12/22/2016 Changing 3rd argument of NewPsi from long to GENERIC everywhere
  12/22/2016 Changing 3rd argument of bk_stack_add_int_attr from long to GENERIC everywhere
*/
...
/*! \fn static int x_handle_error(Display *display,XErrorEvent *x_error)
  \brief x_handle_error
  \param display - Display *display
  \param x_error - XErrorEvent *x_error
...
/*! \fn static int x_handle_fatal_error(Display *display)
  \brief x_handle_fatal_error
  \param display - Display *display
...
/*! \fn void bk_stack_add_int_attr(ptr_psi_term t,char * attrname,GENERIC value)
  \brief bk_stack_add_int_attr
  \param t - ptr_psi_term t
  \param attrname - char * attrname
...
void bk_stack_add_int_attr(ptr_psi_term t,char *attrname,GENERIC value)
{
  ...
  perm=(char *)heap_copy_string(attrname);
  n=find(FEATCMP,perm,t->attr_list);
  if(n) {
    t1=(ptr_psi_term)n->data;
    deref_ptr(t1);
    if(!t1->value_3) {
      push_ptr_value(int_ptr,(GENERIC *)&(t1->value_3));
      t1->value_3=heap_alloc(sizeof(REAL));
    }
    *((REAL *)(t1->value_3)) = *(REAL *) value; // debugged 12/11/2016 DJD
  }
  else {
    t1=stack_psi_term(4);
    t1->type=integer;
...
/*! \fn void bk_change_psi_attr(ptr_psi_term t,char *attrname,ptr_psi_term value)
  \brief bk_change_psi_attr
  \param t - ptr_psi_term t
  \param attrname - char *attrname
  \param value - ptr_psi_term value
...
void bk_change_psi_attr(ptr_psi_term t,char *attrname,ptr_psi_term value)
{
  ...
  if(n) {
    ...
    deref_ptr(t1);
    *t1= *value;
    /*push_ptr_value(psi_term_ptr,(GENERIC *)&(t1->coref));*/
    if(value!=t1)
      value->coref=t1;
  }
  else
    bk_stack_insert(FEATCMP,perm,&(t->attr_list),(GENERIC)value);
}

/*! \fn long unify_int_result(ptr_psi_term t,long v)
  \brief unify_int_result
  \param t - ptr_psi_term t
  \param v - long v
...
/*! \fn static ptr_psi_term NewPsi(ptr_definition t,char *f,GENERIC v)
  \brief ptr_psi_term NewPsi
...
static ptr_psi_term NewPsi(ptr_definition t,char *f, GENERIC v) // changed 12/22/2016
{
  ptr_psi_term p;
...
/*! \fn long GetIntAttr(ptr_psi_term psiTerm,char *attributeName)
  \brief GetIntAttr
  \param psiTerm - ptr_psi_term psiTerm
  \param attributeName - char *attributeName
...
/*! \fn ptr_psi_term GetPsiAttr(ptr_psi_term psiTerm,char *attributeName)
  \brief GetPsiAttr
  \param psiTerm - ptr_psi_term psiTerm
  \param attributeName - char *attributeName
...
/*! \fn static void ResizePixmap(ptr_psi_term psi_window,Display *display,Window window,unsigned long width,unsigned long height)
  \brief ResizePixmap
  \param psi_window - ptr_psi_term psi_window
  \param display - Display *display
  \param window - Window window
  \param width - unsigned long width
  \param height - unsigned long height
...
static void ResizePixmap(ptr_psi_term psi_window,Display *display,Window window,unsigned long width,unsigned long height)
{
  ...
  if((pixmap = XCreatePixmap(display,window,
			       ...
			       attr.depth)) != 0)
    {
      bk_stack_add_int_attr(psiPixmap,"id",(GENERIC)pixmap);
      ...
      /*  RM: Jun 24 1993  */
      if(psiPixmapGC)
	bk_stack_add_int_attr(psiPixmapGC,"id",(GENERIC)pixmapGC); 
      else
	psiPixmapGC=NewPsi(xgc,"id",(GENERIC)pixmapGC); 
      bk_change_psi_attr(psiPixmap,"graphic_context",psiPixmapGC);
    }
}


/*! \fn static void FreeWindow(Display *display,ptr_psi_term psi_window)
  \brief FreeWindow
  \param display - Display *display
  \param psi_window - ptr_psi_term psi_window
...
static void FreeWindow(Display *display,ptr_psi_term psi_window)
{
  ...
  x_free_display_list((ListHeader *)WindowDisplayList(psi_window));
...
/*! \fn long xcOpenConnection()
  \brief xcOpenConnection
...
long xcOpenConnection()
{
  ...
  if(connection = XOpenDisplay(display))
    {
      psiConnection = NewPsi(xdisplay,"id",(GENERIC)connection); 
...
/*! \fn long xcDefaultRootWindow()
  \brief xcDefaultRootWindow
...
long xcDefaultRootWindow()
{
  ...
  psiRoot = NewPsi(xwindow,"id",(GENERIC)DefaultRootWindow(display));
...
/*! \fn static long GetConnectionAttribute(Display *display,long attributeId,long *attribute)
  \brief GetConnectionAttribute
  \param display - Display *display
  \param attributeId - long attributeId
  \param attribute - long *attribute
...
static long GetConnectionAttribute(Display *display,long attributeId,long *attribute)
{
  switch(attributeId) 
    ...
    case 1: 
#ifndef __alpha
      *attribute = display->proto_major_version;
#endif
      break;
    case 2: 
#ifndef __alpha
...
/*! \fn long xcGetConnectionAttribute()
  \brief xcGetConnectionAttribute
...
/*! \fn static long GetScreenAttribute(Display *display,long screen,long attributeId,long *attribute)
  \brief GetScreenAttribute
  \param display - Display *display
  \param screen - long screen
  \param attributeId - long attributeId
  \param attribute - long *attribute
...
/*! \fn long xcGetScreenAttribute()
  \brief xcGetScreenAttribute
...
/*! \fn long xcCloseConnection()
  \brief xcCloseConnection
...
/*! \fn long xcCreateSimpleWindow()
  \brief xcCreateSimpleWindow
...
long xcCreateSimpleWindow()
{
  ...
  if(window = XCreateSimpleWindow(DISP(0),WIND(1),/* display,parent */
				    ...
				    val[6]))        /* BackGround */
    {
      ...
      bk_stack_add_int_attr(psiWindow,"id",(GENERIC)window); 
      ...
      if(!permanent)
	{
	  push_window(destroy_window,(long)DISP(0),window);
	  x_window_creation = TRUE;
	}
      else
	if(show)
	  push_window(show_window,(long)DISP(0),window);
...
*/  
/*! \fn long xcCreateWindow()
  \brief xcCreateWindow
...
long xcCreateWindow()
{
  ...
  if(window = XCreateWindow(DISP(0),WIND(1),/* display,parent */
	...
	x_show_window(DISP(0),window);
      ...
      gcvalues.join_style = JoinRound;
      gc = XCreateGC(DISP(0),window,GCJoinStyle|GCCapStyle,&gcvalues);
      bk_stack_add_int_attr(args[12],"gc",(GENERIC)gc);
...
/*! \fn long xcSetStandardProperties()
  \brief xcSetStandardProperties
...
long xcSetStandardProperties()
{
  ...
  ResizePixmap(args[1],(Display *)val[0],val[1],val[6],val[7]);
...
/*! \fn long xcGetWindowGeometry()
  \brief xcGetWindowGeometry
...
/*! \fn static long GetWindowAttribute(Display *display,long window,long attributeId,long *attribute)
  \brief GetWindowAttribute
  \param display - Display *display
...
/*! \fn long xcGetWindowAttribute()
  \brief xcGetWindowAttribute
...
/*! \fn long xcSetWindowGeometry()
  \brief xcSetWindowGeometry
...
long xcSetWindowGeometry()
{
  ...
  /* modify the pixmap */
  ResizePixmap(args[1],(Display *)val[0],val[1],val[4],val[5]);
...
/*! \fn static long SetWindowAttribute(ptr_psi_term psi_window,Display *display,Drawable window,unsigned long attributeId,unsigned long attribute)
  \brief SetWindowAttribute
  \param psi_window - ptr_psi_term psi_window
  \param display - Display *display
  \param window - Drawable window
  \param attributeId - unsigned long attributeId
  \param attribute - unsigned long attribute
...
/*! \fn long xcSetWindowAttribute()
  \brief xcSetWindowAttribute
...
long xcSetWindowAttribute()
{
  ...
  if(SetWindowAttribute(args[1],(Display *)val[0],val[1],val[2],val[3]))
    {
      XSync(DISP(0),0);
      success = TRUE;
    }
...
/*** RM 8/12/92 START ***/
/*! \fn long xcMapSubwindows()
  \brief xcMapSubwindows
...
/*! \fn long xcUnmapSubwindows()
  \brief xcUnmapSubwindows
...
/*! \fn long xcResizeWindowPixmap()
\brief xcResizeWindowPixmap
...
long xcResizeWindowPixmap()
{
  ...
  /* modify the pixmap */
  ResizePixmap(args[1],(Display *)val[0],val[1],val[2],val[3]);
...
/*! \fn long xcRefreshWindow()
  \brief xcRefreshWindow
...
/*! \fn long xcPostScriptWindow()
  \brief xcPostScriptWindow
...
long xcPostScriptWindow()
{
  ...
  success = x_postscript_window((Display *) val[0],val[1],(ListHeader *)GetIntAttr(GetPsiAttr(args[1],"display_list"),"id"),(char *)val[2]);
  
  end_builtin();
}

/*! \fn long xcDestroyWindow()
  \brief xcDestroyWindow
...
long xcDestroyWindow()
{
  ...
  if(!strcmp(psi->type->keyword->symbol,"true"))
    ...
    {
      FreeWindow((Display *)val[0],args[1]);
...
/*! \fn static long GetGCAttribute(GC gc,long attributeId,long *attribute)
  \brief GetGCAttribute
  \param gc - GC gc
  \param attributeId - long attributeId
  \param attribute - long *attribute
...
/*! \fn long xcGetGCAttribute()
  \brief xcGetGCAttribute
...
long xcGetGCAttribute()
{
  ...
  if(GetGCAttribute((GC)DISP(0),(long)GCVAL(1),&attr))
    {
      unify_real_result(args[2],(REAL) attr);
      success = TRUE;
    }
...
/*! \fn static long SetGCAttribute(Display *display,GC gc,long attributeId,long attribute)
  \brief SetGCAttribute
  \param display - Display *display
  \param gc - GC gc
  \param attributeId - long attributeId
  \param attribute - long attribute
...
/*! \fn long xcSetGCAttribute()
  \brief xcSetGCAttribute
...
long xcSetGCAttribute()
{
  ...
  if(SetGCAttribute((Display *)DISP(0),GCVAL(1),val[2],val[3]))
    success = TRUE;
  else
    {
...
/*! \fn long xcRequestColor()
  \brief xcRequestColor
...
/*! \fn long xcRequestNamedColor()
  \brief xcRequestNamedColor
...
/*! \fn long xcDrawRectangle()
  \brief xcDrawRectangle
...
/*! \fn long xcFillRectangle()
  \brief xcFillRectangle
...
/*! \fn long xcDrawImageString()
  \brief xcDrawImageString
...
/*! \fn static ptr_psi_term xcEventToPsiTerm(XEvent *event)
  \brief xcEventToPsiTerm
  \param event - XEvent *event
...
static ptr_psi_term xcEventToPsiTerm(XEvent *event)
{
  ...
  bk_stack_add_int_attr(psiEvent,"display",(GENERIC)event->xany.display);
  bk_stack_add_int_attr(psiEvent,"window",(GENERIC)event->xany.window);
  
  switch(event->type) {
  case KeyPress:
  case KeyRelease:
    psiEvent->type = xkeyboard_event;
    bk_stack_add_int_attr(psiEvent,"x",(GENERIC)event->xkey.x);
    bk_stack_add_int_attr(psiEvent,"y",(GENERIC)event->xkey.y);
    bk_stack_add_int_attr(psiEvent,"state",(GENERIC)event->xkey.state);
    
    buffer[0] = 0;
    *str = 0;
    XLookupString((XKeyEvent*)event,buffer,sizeof(buffer),&keysym,NULL);
    bk_stack_add_int_attr(psiEvent,"keycode",(GENERIC)&buffer[0]); // added & 12/22/2016
	...
	*str = BS;
    ...
    bk_stack_add_int_attr(psiEvent,"char",(GENERIC)str); // removed * 12/22/2016
    break;
    
  case ButtonPress:
  case ButtonRelease:
    psiEvent->type = xbutton_event;
    bk_stack_add_int_attr(psiEvent,"x",(GENERIC)event->xbutton.x);
    bk_stack_add_int_attr(psiEvent,"y",(GENERIC)event->xbutton.y);
    bk_stack_add_int_attr(psiEvent,"x_root",(GENERIC)event->xbutton.x_root);
    bk_stack_add_int_attr(psiEvent,"y_root",(GENERIC)event->xbutton.y_root);
    bk_stack_add_int_attr(psiEvent,"state",(GENERIC)event->xbutton.state);
    bk_stack_add_int_attr(psiEvent,"button",(GENERIC)event->xbutton.button);
    break;
    
  case Expose:
    psiEvent->type = xexpose_event;
    bk_stack_add_int_attr(psiEvent,"width",(GENERIC)event->xexpose.width);
    bk_stack_add_int_attr(psiEvent,"height",(GENERIC)event->xexpose.height);
    break;
  ...
  case ConfigureNotify:
    ...
    bk_stack_add_int_attr(psiEvent,"x",(GENERIC)event->xconfigure.x);
    bk_stack_add_int_attr(psiEvent,"y",(GENERIC)event->xconfigure.y);
    bk_stack_add_int_attr(psiEvent,"width",(GENERIC)event->xconfigure.width);
    bk_stack_add_int_attr(psiEvent,"height",(GENERIC)event->xconfigure.height);
    bk_stack_add_int_attr(psiEvent,"border_width",
		       (GENERIC)event->xconfigure.border_width);
    break;
    /*** RM 7/12/92(END) ***/
  ...
  LeaveEnterCommon:
    bk_stack_add_int_attr(psiEvent,"root",    (GENERIC) event->xcrossing.root);
    bk_stack_add_int_attr(psiEvent,"subwindow",(GENERIC)event->xcrossing.subwindow);
    
    bk_stack_add_int_attr(psiEvent,"x",(GENERIC)event->xcrossing.x);
    bk_stack_add_int_attr(psiEvent,"y",(GENERIC)event->xcrossing.y);
    
    bk_stack_add_int_attr(psiEvent,"focus",(GENERIC)event->xcrossing.focus);
    bk_stack_add_int_attr(psiEvent,"state",(GENERIC)event->xcrossing.state);
  ...
  default:
    ...
    bk_stack_add_int_attr(psiEvent,"event_type",(GENERIC)event->type);
    break;
  }
...
/*! \fn long list_is_nil(ptr_psi_term(lst))
  \brief list_is_nil
  \param ptr_psi_term(lst)
  ...
  HAD BEEN long list_is_nil(ptr_psi_term(lst)) changed 2.30 DJD 12/22/2016
*/
...
/*! \fn ptr_psi_term list_cdr(ptr_psi_term lst)
  \brief list_cdr
  \param lst ptr_psi_term lst

  HAD BEEN ptr_psi_term list_cdr(ptr_psi_term(lst)) changed 2.30 DJD 12/22/2016
*/
...
/*! \fn ptr_psi_term list_car(ptr_psi_term lst)
  \brief list_car
  \param lst ptr_psi_term lst

  HAD BEEN ptr_psi_term list_car(ptr_psi_term(lst)) changed 2.30 DJD 12/22/2016
*/
...
/*! \fn void list_set_car(ptr_psi_term lst,ptr_psi_term value)
  \brief list_set_car
  \param lst - ptr_psi_term lst
  \param value - ptr_psi_term value
...
/*! \fn void list_set_cdr(ptr_psi_term lst,ptr_psi_term value)
  \brief list_set_cdr
  \param lst - ptr_psi_term lst
  \param value - ptr_psi_term value
...
/*! \fn ptr_psi_term list_last_cdr(ptr_psi_term lst)
  \brief list_last_cdr
  \param lst - ptr_psi_term lst
...
/*! \fn ptr_psi_term append_to_list(ptr_psi_term lst,ptr_psi_term value)
  \brief append_to_list
  \param lst - ptr_psi_term lst
  \param value - ptr_psi_term value
...
/*! \fn long map_funct_over_list(ptr_psi_term lst,long(*proc)(),long *closure)
  \brief map_funct_over_list
  \param lst - ptr_psi_term lst
  \param proc - long(*proc)()
...
/*! \fn long map_funct_over_cars(ptr_psi_term lst,long(*proc)(),long *closure)
  \brief map_funct_over_cars
  \param lst - ptr_psi_term lst
  \param proc - long(*proc)()
  \param closure - long *closure
...
/*! \fn void list_remove_value(ptr_psi_term lst,ptr_psi_term value)
  \brief list_remove_value
  \param lst - ptr_psi_term lst
  \param value - ptr_psi_term value
...
/*! \fn static long x_union_event(ptr_psi_term psiEvent,EventClosure *closure)
  \brief x_union_event
  \param psiEvent - ptr_psi_term psiEvent
  \param closure - EventClosure *closure
...
long xcGetEvent()
{
  ...
  if(!xevent_existing) {
    ...
    bk_stack_add_int_attr(eventElt,"display",(GENERIC)val[0]);
    bk_stack_add_int_attr(eventElt,"window",(GENERIC)val[1]);
    bk_stack_add_int_attr(eventElt,"mask",(GENERIC)val[2]);

    /* stack_insert(FEATURECMP,"event",&(eventElt->attr_list),(GENERIC)result); */
...
/*! \fn static long x_flush_event(ptr_psi_term eventElt,EventClosure *closure)
  \brief x_flush_event
  \param eventElt - ptr_psi_term eventElt
  \param closure - EventClosure *closure
...
static long x_flush_event(ptr_psi_term eventElt,EventClosure *closure)
{
  ...
  if ((Display *)GetIntAttr(psiEvent,"display") == closure->display
       ...
       && (GetIntAttr(psiEvent,"mask") & closure->mask) != 0)
    {
      ...
      /*     push_ptr_value_global(psi_term_ptr,(GENERIC *)&xevent_list); */
...
long xcSendEvent()
{
  ...
  if(xcPsiEventToEvent(val[2],&event))
    {
      XSendEvent((GENERIC)DISP(0),WIND(1),False,?,&event);
      success = TRUE;
    }
  else
    {
...
/*! \fn long xcQueryPointer()
  \brief xcQueryPointer
...
/*! \fn void x_setup_builtins()
  \brief x_setup_builtins
...
/*! \fn static long WaitNextEvent(long *ptreventflag)
  \brief WaitNextEvent
  \param ptreventflag - long *ptreventflag
...
/*! \fn long x_read_stdin_or_event(long *ptreventflag)
  \brief x_read_stdin_or_event
  \param ptreventflag - long *ptreventflag
...
long x_read_stdin_or_event(long *ptreventflag)
{
  ...
  *ptreventflag = FALSE;
  ...
  if(c = saved_char) /* not an error ;-) */
    ...
    {
      if(input_stream == NULL || feof(input_stream))
	c = EOF;
      else 
	{
	  if(start_of_line) 
...
/*! \fn static long mask_match_type(long mask,long type)
  \brief mask_match_type
...
/*! \fn static ptr_psi_term x_what_psi_event(ptr_psi_term beginSpan,ptr_psi_term endSpan,long eventType)
  \brief x_what_psi_event
  \param beginSpan - ptr_psi_term beginSpan
  \param endSpan - ptr_psi_term endSpan
...
/*! \fn static void x_build_existing_event(XEvent *event,ptr_psi_term beginSpan,ptr_psi_term endSpan,long eventType)
  \brief x_build_existing_event
  \param event - XEvent *event
  \param beginSpan - ptr_psi_term beginSpan
  \param endSpan - ptr_psi_term endSpan
  \param eventType - long eventType
...
/*! \fn static long x_next_event_span(ptr_psi_term eventElt,EventClosure *eventClosure)
  \brief x_next_event_span
  \param eventElt - ptr_psi_term eventElt
  \param eventClosure - EventClosure *eventClosure
...
/*! \fn void x_destroy_window(Display *display,Window window)
  \brief x_destroy_window
  \param display - Display *display
  \param window - Window window
...
/*! \fn void x_show_window(Display *display,long window)
  \brief x_show_window
  \param display - Display *display
  \param window - long window
...
/*! \fn void x_hide_window(Display *display,long window)
  \brief x_hide_window
  \param display - Display *display
  \param window - long window
...
/*! \fn void x_show_subwindow(Display *display,long window)
  \brief x_show_subwindow
  \param display - Display *display
  \param window - long window
...
/*! \fn void x_hide_subwindow(Display *display,long window)
  \brief x_hide_subwindow
  \param display - Display *display
  \param window - long window
...
/*! \fn long xcQueryTextExtents()
  \brief xcQueryTextExtents
...
/*! \fn ptr_goal GoalFromPsiTerm(ptr_psi_term psiTerm)
  \brief GoalFromPsiTerm
  \param psiTerm - ptr_psi_term psiTerm
