...
/*! \fn void bk_stack_add_int_attr(ptr_psi_term t,char * attrname,long value)
...
/*! \fn static ptr_psi_term NewPsi(ptr_definition t,char *f,long v)
...
static void ResizePixmap(ptr_psi_term psi_window,Display *display,Window window,unsigned long width,unsigned long height)
{
  ...
  if((pixmap = GetIntAttr(psiPixmap,"id")) != 0)
	...
	XFreeGC(display,pixmapGC);
      ...
      bk_stack_add_int_attr(psiPixmap,"id",(long)NULL);
      bk_stack_add_int_attr(psiPixmapGC,"id",(long)NULL);
    }
  ...
  if((pixmap = XCreatePixmap(display,window,
			       ...
			       attr.depth)) != 0)
    {
      bk_stack_add_int_attr(psiPixmap,"id",(long)pixmap);
      ...
      /*  RM: Jun 24 1993  */
      if(psiPixmapGC)
	bk_stack_add_int_attr(psiPixmapGC,"id",(long)pixmapGC); // add cast DJD
      else
	psiPixmapGC=NewPsi(xgc,"id",(long)pixmapGC); // add cast DJD
...
long xcOpenConnection()
{
  ...
  if(connection = XOpenDisplay(display))
    {
      psiConnection = NewPsi(xdisplay,"id",(long)connection); // add cast DJD
...
long xcDefaultRootWindow()
{
  ...
  psiRoot = NewPsi(xwindow,"id",(long)DefaultRootWindow(display)); // added cast DJD
...
long xcGetConnectionAttribute()
{
  ...
  types[0] = xdisplay;   // changed from real 12/24/2016 DJD stopped X crash
  types[1] = real;
  types[2] = xdrawable;  // changed from real 12/24/2016 DJD
...
long xcCreateSimpleWindow()
{
  ...
  if(window = XCreateSimpleWindow(DISP(0),WIND(1),/* display,parent */
				    ...
				    val[6]))        /* BackGround */
    {
      ...
      bk_stack_add_int_attr(psiWindow,"id",(long)window); // added cast DJD
...
static ptr_psi_term xcEventToPsiTerm(XEvent *event)
{
  ...
  case KeyRelease:
    ...
    *str = 0;
    ...
    bk_stack_add_int_attr(psiEvent,"keycode",(long)buffer[0]);
	...
	*str = BS;
    ...
    bk_stack_add_int_attr(psiEvent,"char",(long)*str);
    break;
    
  case ButtonPress:
  case ButtonRelease:
    psiEvent->type = xbutton_event;
    bk_stack_add_int_attr(psiEvent,"x",(long)event->xbutton.x);
    bk_stack_add_int_attr(psiEvent,"y",(long)event->xbutton.y);
    bk_stack_add_int_attr(psiEvent,"x_root",(long)event->xbutton.x_root);
    bk_stack_add_int_attr(psiEvent,"y_root",(long)event->xbutton.y_root);
    bk_stack_add_int_attr(psiEvent,"state",(long)event->xbutton.state);
    bk_stack_add_int_attr(psiEvent,"button",(long)event->xbutton.button);
    break;
    
  case Expose:
    psiEvent->type = xexpose_event;
    bk_stack_add_int_attr(psiEvent,"width",(long)event->xexpose.width);
    bk_stack_add_int_attr(psiEvent,"height",(long)event->xexpose.height);
    break;
  ...
  case ConfigureNotify:
    ...
    bk_stack_add_int_attr(psiEvent,"x",(long)event->xconfigure.x);
    bk_stack_add_int_attr(psiEvent,"y",(long)event->xconfigure.y);
    bk_stack_add_int_attr(psiEvent,"width",(long)event->xconfigure.width);
    bk_stack_add_int_attr(psiEvent,"height",(long)event->xconfigure.height);
  ...
  LeaveEnterCommon:
    bk_stack_add_int_attr(psiEvent,"root",    (long) event->xcrossing.root);
    bk_stack_add_int_attr(psiEvent,"subwindow",(long)event->xcrossing.subwindow);
    
    bk_stack_add_int_attr(psiEvent,"x",(long)event->xcrossing.x);
    bk_stack_add_int_attr(psiEvent,"y",(long)event->xcrossing.y);
    
    bk_stack_add_int_attr(psiEvent,"focus",(long)event->xcrossing.focus);
    bk_stack_add_int_attr(psiEvent,"state",(long)event->xcrossing.state);
  ...
  default:
    ...
    bk_stack_add_int_attr(psiEvent,"event_type",(long)event->type);
    break;
  }
...
long xcGetEvent()
{
  ...
  if(!xevent_existing) {
    ...
    bk_stack_add_int_attr(eventElt,"display",(long)val[0]);
    bk_stack_add_int_attr(eventElt,"window",(long)val[1]);
    bk_stack_add_int_attr(eventElt,"mask",(long)val[2]);
